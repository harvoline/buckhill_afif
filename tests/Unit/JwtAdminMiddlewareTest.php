...
class JwtAdminMiddlewareTest extends TestCase
{
    use RefreshDatabase;
    ...
    /** @test */
    public function non_admins_are_blocked()
    {
        $user = User::factory()
            ->asUser()
            ->create();
        ...
        $request = Request::create('/api/v1/admin/user-list', 'GET');

        $request->headers->set('Authorization', $token);

        $middleware = new JwtAdminMiddleware;
    ...
    /** @test */
    public function admins_are_not_blocked()
    {
        $user = User::factory()
            ->asAdmin()
            ->create();
